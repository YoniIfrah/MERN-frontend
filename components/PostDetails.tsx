...
const getPostByid = async (id:string) =>{
  id = id.replace(/"/g, '');
  const postData:Student = await StudentModel.getStudentById(id)
  return postData
}
...
const PostDetails : FC<{route:any, navigation: any }> = ({route, navigation}) => {
  ...
  return (
    ...
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        { editable == true && <EditPost email = {PostData?.email || ''} text={PostData?.name || ''} avatarUrl={PostData?.avatarUrl || ''}></EditPost> }
